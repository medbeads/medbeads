"""Retriever interface + 4-arm implementation (R8.2, specs/DESIGN_v3.md §9).
...
Bead is the unifying unit across all four (docs/requirements.md R8.2's
"チャンク = Bead に統一").
...
Arms (bench/bench/retrieval/):
  ...
  - ``dag.py`` (DagRetriever, ``include_siblings`` toggles dag_nosib vs.
    dag_full): the unified ``retrieve`` MCP tool with semantic=True, using
    its own token-budgeted L0/L1/L2 packing (graph.BuildContext) rather than
    this module's greedy-L0-only packing.
