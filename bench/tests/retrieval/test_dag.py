...
def test_retrieve_passes_include_siblings_and_semantic_true() -> None:
    client = _FakeClient({"items": [], "used_tokens": 0, "anchor_ids": [], "truncated_refs": []})
    retriever = DagRetriever(client, include_siblings=False, chain_depth=7)
...
def test_arm_name_reflects_include_siblings() -> None:
    client = _FakeClient({"items": []})
    assert DagRetriever(client, include_siblings=False).arm == "dag_nosib"
    assert DagRetriever(client, include_siblings=True).arm == "dag_full"
...
def test_bead_ids_and_texts_come_from_items_in_order() -> None:
    ...
    retriever = DagRetriever(client, include_siblings=True)
    ...
    assert result.meta["include_siblings"] is True
...
def test_duplicate_item_ids_are_deduplicated_keeping_first_occurrence() -> None:
    ...
    out = {
        "items": [
            ...
            {"id": "sha256:x", "text": "second (sibling) occurrence", "granularity": "L1", "provenance": "sibling"},
        ],
        "used_tokens": 50,
        "anchor_ids": ["sha256:y"],
        "truncated_refs": [],
    ...
    # First occurrence (ancestor) wins, not the later (sibling) duplicate.
...
def test_missing_items_key_yields_empty_result() -> None:
    client = _FakeClient({})
    retriever = DagRetriever(client, include_siblings=False)
