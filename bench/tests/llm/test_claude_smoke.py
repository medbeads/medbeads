...
one scenario x 2-arm (rag, dag_full) smoke, asserting an answer comes back
and usage is recorded in the JSONL transcript — nothing more.
"""
...
def test_real_claude_answers_for_rag_and_dag_full(tmp_path: Path) -> None:
    ...
    assert {r["arm"] for r in rows} == {"rag", "dag_full"}
    for row in rows:
