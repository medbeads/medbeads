...
// TestRun_ApcSubcommandRemoved checks U5a's removal (specs/U5_api_retrieve.md):
// the old `apc` subcommand no longer exists at all — it must be rejected the
// same way any other unknown command is, not silently accepted.
func TestRun_ApcSubcommandRemoved(t *testing.T) {
	devNull, err := os.OpenFile(os.DevNull, os.O_RDWR, 0)
	if err != nil {
		t.Fatalf("open %s: %v", os.DevNull, err)
	}
	...
	dir := t.TempDir()
	if got := run([]string{"apc", "-data", dir}, devNull, devNull); got != 1 {
		t.Errorf("run(apc -data %s) = %d, want 1 (apc subcommand removed, should be an unknown-command error)", dir, got)
	}
}
